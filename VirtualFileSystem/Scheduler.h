...
/**
*	\brief A Scheduler manages all jobs on the system.  Jobs are either running, 
*	waiting or finished.  The scheduler will only run a single process at a 
*	time.  The process will run for a number of ticks equal to the burst time.  
*	If the job finishes before the burst, it will be moved to the finished queue 
*	and a new job/burst is started.  If the job is not finished within the 
*	burst, it is pushed onto the end of the running queue and a new job is 
*	pulled off.  If a job needs to seek IO while running, it will be taken off 
*	the queue entirely until it has finished it's IO.
*/
class Scheduler
{
public:
	 ...
	 *	
	 *	The default burst and memory are invalid to begin with.  The user must 
	 *	set them by calling setMemory and setBurst.  This was done as 
	 ...
	 *	
	 *	The system will run in bursts of time.  A process is pulled from the 
	 *	running queue and ticked the number of times allowed by the burst.  
	 *	It is then sent to the end of the queue.
	 *	
	 *	If the job needs IO, it is moved off the running queue and into the IO 
	 *	queue where it will run in parallel with the rest of the jobs.
	 *	
	...
	/**
	 *	\brief Scan the getting IO queue to see if any of the jobs are down 
	 *	with IO.
	 *	
	 *	If a job on the IO queue has finished getting IO, it will be moved to 
	 *	the end of the running queue.  If it is not finished, it will just be 
	 *	kept on the IO queue.
	 */
	 ...
	 *	
	 *	This is similar to calling the run function.  However, instead of 
	 *	running until everything is finished, the system is only ticked a 
	 ...
	 *	requires 2 ticks to be loaded from VM.  If the burst time is less than 
	 *	2, jobs will never get loaded.
	 */
	...
	/**
	 *	\return The amount of time the scheduler will allocate to a job before 
	 *	it is swapped.
	 */
	int getBurst();
	 ...
	 *	
	 *	\param program A ProgramFile that should be converted into a process 
	 *	and added to the scheduler.
	 *	
	 *	The ProgramFile will be checked to ensure that the system has enough 
	 *	memory to run the Program.  If it does, the Program will be converted 
	 *	into a process.  This process will be allocated memory.  If there is 
	 *	not enough available, the scheduler will use the RMU algorithm to shift 
	 *	a process into VM.
	 */
	void addProcess( ProgramFile* program );

	/**
	 *	\brief Attempt to acquire resources for a process.  This is done by 
	 *	shifting jobs into VM which frees their memory.
	 *	
	 *	\param amount How much memory needs to be acquired
	 *	
	 *	There is a chance that the required memory cannot be acquired.  If 
	 *	there are jobs getting IO, they are not considered for a VM shift.  
	 *	Likewise, a job that is currently running cannot be moved to VM.
	 *	
...
private:
	 ...
	 *	to the finished queue, if it needs IO it will be sent to that queue.  
	 *	If it has exhausted it's burst, it will be pushed to the back of the 
	 *	running queue.
	 *	
	...
	/**
	 *	\brief A process has finished working, move it to the finished queue 
	 ...
	 *	running queue.
	 *	
	 *	Only jobs that are currently on the running queue but that are not 
	 *	actually running are considered for freeing.  A job that is already in 
	 *	VM is also ignored.
	 *	
	 ...
	 *	memory required.
	 *	
	 *	This uses the MRU algorithm to shift jobs into the VM.  This is not the
	 *	most efficient way of doing it, as it might free more memory than 
	 *	needed.
	 *	
	 *	\param amount How much memory was actually freed.  This may be more 
	 *	than was requested.
	 */
